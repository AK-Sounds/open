(() => {
  ...
  function scheduler() {
    ...
    while (nextTimeA < now + 0.5) {
      ...
      // FIXED: GRAVITY (Using Global patternIdxA)
      if (!isCadence) {
          const anchor = 1; 
  ...
  async function renderWavExport() {
    ...
    const a = document.createElement('a'); a.style.display = 'none'; a.href = url; a.download = `open-final-v136-${Date.now()}.wav`;
